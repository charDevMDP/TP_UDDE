...
CREATE PROCEDURE billing_measurement_client (IN id_meter INTEGER)
BEGIN
    ...
    INSERT INTO invoices (id_meters, date_invoice , date_initial) VALUES (id_meter, @datee, @due_date);
   ...
   OPEN cur_invoices;
    ...
    #asigno el total_price, cost_price y measurement_number a la factura
    UPDATE invoices i SET i.consumer_kw = consumer_kw_sum , i.total = cost_sum, i.number = measurement_number
   	WHERE i.id =  id_invoices;

    #si la medidor nunca registro mediciones, pongo el estado de la factura en PAGO.
    IF EXISTS(SELECT i.id_meters  FROM invoices i WHERE i.id = id AND i.number = 0) THEN
    	BEGIN
...
CALL  billing_measurement_client("6");